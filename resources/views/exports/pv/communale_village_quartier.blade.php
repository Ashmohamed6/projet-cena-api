...
<head>
    <meta charset="UTF-8">
    <title>PV Village/Quartier - Communales</title>
    <style>
        @page {
...
<body>
    ...
    <div class="localisation-box">
        ...
        <table class="localisation-table">
            ...
            <tr>
                ...
                <td>{{ strtoupper($localisation['arrondissement'] ?? '') }}</td>
            </tr>
            <tr>
                <td>VILLAGE/QUARTIER DE VILLE :</td>
    ...
    <div class="text-justify">
        ...
        ai procédé à la compilation des résultats de tous les postes de vote, centre de vote par centre de vote pour obtenir les 
        résultats du village ou du quartier de ville.
    </div>

    <div class="banner">
        VILLAGE OU QUARTIER DE VILLE : {{ strtoupper($localisation['departement'] ?? '') }} / {{ strtoupper($localisation['commune'] ?? '') }} / {{ strtoupper($localisation['arrondissement'] ?? '') }} / {{ strtoupper($localisation['village_quartier'] ?? '') }}
    </div>

    @php
        $lignesParCentre = [];
        foreach ($lignes as $ligne) {
            if ($ligne['type'] === 'poste_vote' && isset($ligne['centre_vote_nom'])) {
                $centreNom = $ligne['centre_vote_nom'];
                $lignesParCentre[$centreNom][] = $ligne;
            }
        }
    @endphp

    @foreach($lignesParCentre as $centreNom => $postesLignes)
        <div class="centre-title">CENTRE DE VOTE : {{ strtoupper($centreNom) }}</div>
            ...
            <tbody>
                ...
                <tr class="total-row">
                    ...
                    @foreach($entites as $entite)
                        ...
                        <td>{{ $totalCentre }}</td>
                    @endforeach
                    <td>{{ collect($postesLignes)->sum('bulletins_nuls') }}</td>
                    <td></td>
                </tr>
            </tbody>
        </table>
